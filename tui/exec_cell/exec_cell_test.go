...
func TestExecCellNonExploringCommandsRenderIndividuallyLikeRust(t *testing.T) {
	cell := NewExecCell(ExecCall{
		CallID:  "call-first",
	...
	// Rust #41893: a completed non-exploring command flushes and renders on its
	// own; it never accumulates into a compact summary with the next command.
	if !cell.ShouldFlush() {
		t.Fatal("a completed non-exploring cell must flush")
	}
	if _, ok := cell.WithAddedCall("call-second", []string{"bash", "-lc", "printf second"}, nil, ExecSourceAgent, ""); ok {
		t.Fatal("a second command must not join a non-exploring cell")
	}
	display := strings.Join(cell.DisplayLines(80), "\n")
	if !strings.Contains(display, "printf first") || strings.Contains(display, "Ran 1 command") {
		t.Fatalf("completed command display:\n%s", display)
	}
}

// Rust #46487: adjacent read/list/search calls stay grouped, including after a
// failed read, and never flush while the group is exploring.
func TestExecCellExploringGroupStaysOpenLikeRust(t *testing.T) {
	cell := NewExecCell(ExecCall{
		CallID:  "read-1",
	...
	if !cell.CompleteCall("read-1", CommandOutput{ExitCode: 1, AggregatedOutput: "missing\n"}, time.Second) {
		t.Fatal("CompleteCall failed")
	}
	if cell.ShouldFlush() {
		t.Fatal("exploration stays open after a failed read")
	}
	next, ok := cell.WithAddedCall("search-1", []string{"bash", "-lc", "rg TODO"}, []ParsedCommand{{Kind: ParsedSearch, Query: "TODO"}}, ExecSourceAgent, "")
	if !ok {
		t.Fatal("an adjacent search must join the exploring group")
	}
	if len(next.Calls) != 2 {
		t.Fatalf("exploring calls = %d, want 2", len(next.Calls))
	}
	display := strings.Join(next.DisplayLines(80), "\n")
	if !strings.Contains(display, "Exploring") || !strings.Contains(display, "Read README.md") || !strings.Contains(display, "Search TODO") {
		t.Fatalf("active exploring group display:\n%s", display)
	}
	if !next.CompleteCall("search-1", CommandOutput{ExitCode: 0}, time.Second) {
		t.Fatal("search CompleteCall failed")
	}
	if next.ShouldFlush() {
		t.Fatal("a completed exploring group stays grouped for adjacent calls")
	}
