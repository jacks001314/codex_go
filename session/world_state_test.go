...
func TestWorldStateRoundTripTracksRuntimeInstructionDomains(t *testing.T) {
	...
	if string(state.Model) != `"gpt-5"` || string(state.Personality) != `{"model":"gpt-5","personality":"pragmatic"}` || string(state.CollaborationMode) != `{"mode":"plan"}` || string(state.PermissionInstructions) != `{"profile":"workspace-write"}` || string(state.RealtimeConversation) != `{"active":true}` || string(state.Tools) != `{"mcp__drive":"Drive tools"}` {
