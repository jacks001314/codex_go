...
func (a Action) Parameters() map[string]any {
	integer := func(minimum int, description string) map[string]any {
	...
	nullableString := func(description string) map[string]any {
		return map[string]any{"anyOf": []any{map[string]any{"type": "string"}, map[string]any{"type": "null"}}, "description": description}
	}
	role := func(description string) map[string]any {
		return map[string]any{
			"anyOf": []any{
				map[string]any{"type": "string", "enum": []any{"user", "assistant", "tool", "system", "developer"}},
	...
	switch a {
	case HistoryListWindows:
		properties["limit"] = integer(1, "Maximum number of windows to return.")
	...
	case HistoryListItems:
		properties["limit"] = integer(1, "Maximum number of items to return.")
		...
		properties["max_chars_per_item"] = integer(1, "Maximum characters returned in each item's truncated_content.")
	case HistoryReadItem:
		properties["agent_name"] = nullableString(agentNameDesc)
		...
		properties["offset_chars"] = integer(0, "Zero-based character offset at which reading starts.")
		properties["limit_chars"] = integer(1, "Maximum number of characters to return.")
	...
	case NotesListFilesByPrefix:
		...
		properties["max_results"] = integer(1, "Maximum number of files to return.")
	...
	case NotesReadFile:
		...
		properties["start_line"] = map[string]any{"anyOf": []any{map[string]any{"type": "integer"}, map[string]any{"type": "null"}}, "description": "First line to return, inclusive and 1-based. Negative values count backward from the final line."}
		properties["stop_line"] = map[string]any{"anyOf": []any{map[string]any{"type": "integer"}, map[string]any{"type": "null"}}, "description": "Last line to return, inclusive and 1-based. Negative values count backward from the final line."}
...
// #40775 removed client-side argument limits so valid JSON object arguments are
// forwarded to the backend; non-object arguments are rejected upstream by the
// tool argument decoder.
