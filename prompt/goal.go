...
func Continuation(goal *Goal) string {
	return renderGoalTemplate(goalContinuationTemplate, goal)
}

func BudgetLimit(goal *Goal) string {
	return renderGoalTemplate(goalBudgetLimitTemplate, goal)
}
...
func renderGoalTemplate(source string, goal *Goal) string {
	if goal == nil {
		goal = &Goal{}
	}
	remaining := remainingTokens(goal.TokenBudget, goal.TokensUsed)
	if strings.Contains(source, "<untrusted_objective>") && goal.TokenBudget == nil {
		remaining = "unknown"
	}
	replacements := []string{
		"{{ objective }}", escapeXMLText(goal.Objective),
		"{{ tokens_used }}", strconv.FormatInt(goal.TokensUsed, 10),
		"{{ token_budget }}", tokenBudget(goal.TokenBudget),
