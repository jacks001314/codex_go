...
func TestRustTUISnapshotManifestCoversPrioritySurfaces(t *testing.T) {
	...
	if got := countSnapFilesRecursive(t, filepath.Join(root, "tui")); got != 832 {
		t.Fatalf("Rust TUI snapshot total drift: got %d want 832", got)
	}
...
func rustTUISnapshotManifest() []rustTUISnapshotDir {
	return []rustTUISnapshotDir{
		...
		},
		{
			...
			Required: []string{
				"tui/src/bottom_pane/chat_composer/snapshots/codex_tui__bottom_pane__chat_composer__sparkle__tests__astra_current_composer.snap",
			},
		},
		{
