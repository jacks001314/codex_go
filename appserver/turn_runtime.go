...
// CollaborationModeDefaultInstructions exposes the Default-mode developer
// instructions byte-for-byte (no trimming) for the parity shared-fixture
// verifier pinning the template against the Rust collaboration-mode-templates
// blob.
func CollaborationModeDefaultInstructions() string {
	return collaborationModeDefaultInstructions
}
...
func (r *RuntimeRouter) runTurnRuntime(ctx context.Context, params *turn.TurnStartParams, record *turn.TurnRecord, runtime *turn.Runtime, connectionID string) {
	...
	result, err := runtime.Run(ctx, &turn.AgentLoopRequest{
		...
		Tracer:                       r.sessionTelemetryForThread(threadID),
...
func (r *RuntimeRouter) runtimeToolCompletedNotifier(threadID string, turnID string, cwd string, unifiedExecEnabled bool) turn.ToolCompletedCallback {
	return func(ctx context.Context, execution *turn.ToolExecutionResult) {
		...
		mcpOutcome, isMCPCall := telemetry.MCPCallOutcomeForExecution(execution)
		if span := r.takeMCPToolCallSpan(threadID, turnID, execution.Invocation.CallID); span != nil {
			...
			if isMCPCall {
				if attributes := telemetry.MCPCallSpanAttributes(mcpOutcome, telemetry.MCPExecutionResultMeta(execution)); len(attributes) > 0 {
