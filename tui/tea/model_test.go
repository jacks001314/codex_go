...
func TestModelRustSlashLongTailCommandSurfaces(t *testing.T) {
	...
	// Rust #41921: enabling Vim starts the composer in Insert mode, so the next
	// slash command types directly.
...
// Rust #41893/#46487: successful commands render individually instead of a
// `Ran N commands` summary, and a completed command flushes its cell.
func TestModelCompletedCommandsRenderIndividuallyLikeRust(t *testing.T) {
	state := codextui.NewState(nil)
	model := NewModel(state, Options{Width: 80, Height: 24})
	exit0 := 0
	...
	view := utils.StripANSI(model.View())
	if !strings.Contains(view, "Ran printf first") || !strings.Contains(view, "Running printf second") {
		t.Fatalf("individual command cells missing:\n%s", view)
	}
	if strings.Contains(view, "Ran 1 command") || strings.Contains(view, "Ran 2 commands") {
		t.Fatalf("commands must not collapse into a compact summary:\n%s", view)
	}
	model.Update(ThreadEventMsg{Event: protocol.ItemCompleted(commandExecutionItemWithSource("call-2", "printf second", "second\n", &exit0, "completed", "agent"))})
	view = utils.StripANSI(model.View())
	if !strings.Contains(view, "Ran printf second") {
		t.Fatalf("completed command must stay visible:\n%s", view)
	}
	for _, message := range state.Messages {
		raw := message.RawText
		if strings.Contains(raw, "$ printf first") && strings.Contains(raw, "$ printf second") {
			t.Fatalf("commands must not share one history cell: %q", raw)
		}
	}
	...
	view = model.View()
	if !strings.Contains(view, "done ") {
		t.Fatalf("assistant output after the commands should use the final-message separator:\n%s", view)
	}
}

// Rust #46487: a failed command keeps its own cell with its failure status.
func TestModelCommandActivityKeepsFailuresVisible(t *testing.T) {
	state := codextui.NewState(nil)
	model := NewModel(state, Options{Width: 80, Height: 24})
	exit0 := 0
	exit1 := 1
	...
	view := utils.StripANSI(model.View())
	if !strings.Contains(view, "Ran printf broken") {
...
// A manual shell command renders with the "You ran" title, and a turn boundary
// keeps whatever was already rendered.
func TestModelCommandActivityRendersManualShellAndTurnBoundaryLikeRust(t *testing.T) {
	state := codextui.NewState(nil)
	model := NewModel(state, Options{Width: 80, Height: 24})
	exit0 := 0
	...
	view := utils.StripANSI(model.View())
	if !strings.Contains(view, "You ran printf manual") || !strings.Contains(view, "Ran printf first") {
	...
	if view = utils.StripANSI(model.View()); !strings.Contains(view, "Ran printf second") {
