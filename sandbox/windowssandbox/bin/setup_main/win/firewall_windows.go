...
func withFirewallRules(fn func(policy *ole.IDispatch, rules *ole.IDispatch) error) error {
	...
	// COM initialization is scoped to an OS thread. Pin the goroutine for the
	// whole firewall operation so subsequent automation calls cannot migrate to
	// a different, uninitialized thread.
