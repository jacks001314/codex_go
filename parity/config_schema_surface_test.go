...
var configSchemaRustOnlyAllowlist = map[string]string{
	...
	"feedback":                            "Rust feedback-reporting config section; Go has no feedback doctor attachment or config section",
	"ghost_snapshot":                      "Rust checkpoint/ghost snapshot persistence; not implemented in Go",
	"log_dir":                             "Rust log directory override; Go logs to its own layout without a config override",
	...
	"show_raw_agent_reasoning":            "Rust raw reasoning display toggle; Go hides agent reasoning per feature flags",
...
func TestRustConfigSchemaSurfaceAgainstGo(t *testing.T) {
	...
	if len(goKeys) != 84 {
		t.Fatalf("Go recognized top-level config key count = %d, want 84 (pinned baseline)", len(goKeys))
	}
