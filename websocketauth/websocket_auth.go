...
// Shared authentication for incoming app-server and exec-server WebSocket
// upgrades.
//
// Rust parity: the codex-websocket-auth crate (#47447), which the app-server and
// the exec-server both use. Policies retain token digests or JWT verification
// secrets; each listener chooses whether authentication is required.
...
const (
	DefaultMaxClockSkewSeconds uint64 = 30
...
const (
	ModeCapabilityToken   Mode = "capability-token"
	ModeSignedBearerToken Mode = "signed-bearer-token"
)
...
func (p *Policy) IsUnauthenticatedNonLoopbackListener(addr net.Addr) bool {
...
func (p *Policy) Authorize(headers http.Header, now time.Time) error {
	if p == nil || !p.IsConfigured() {
		return nil
	}
...
func newCapabilityTokenPolicy(settings *Settings) (*Policy, error) {
	...
	return &Policy{mode: ModeCapabilityToken, tokenSHA256: digest}, nil
}

func newSignedBearerTokenPolicy(settings *Settings) (*Policy, error) {
...
func (p *Policy) verifySignedBearerToken(token string, now time.Time) error {
...
func (p *Policy) validateJWTClaims(claims *websocketJWTClaims, now time.Time) error {
