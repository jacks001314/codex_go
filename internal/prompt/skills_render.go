...
var SkillsHowToUseWithAbsolutePaths = strings.Join([]string{
	"- Discovery: The list above is the skills available in this session (name + description + source locator). `file` entries live on the host filesystem, `environment resource` entries are owned by an execution environment, `orchestrator resource` entries must be accessed through `skills.list` and `skills.read`, and `custom resource` entries use their provider's access mechanism.",
...
func renderAvailableSkillsBody(skillLines []string) string {
	...
	builder.WriteString("\n")
	builder.WriteString("\n## Skills\n")
	builder.WriteString(SkillsIntroWithAbsolutePaths)
	builder.WriteString("\n")
	builder.WriteString("### Available skills\n")
	builder.WriteString(strings.Join(skillLines, "\n"))
	builder.WriteString("\n### How to use skills\n")
	builder.WriteString(SkillsHowToUseWithAbsolutePaths)
	builder.WriteString("\n")
