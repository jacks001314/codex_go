...
func TestRustModelInfoFieldSurfaceAgainstGo(t *testing.T) {
	...
	if len(rustSet) != 44 {
		t.Fatalf("Rust ModelInfo wire field count = %d, want 44 (pinned baseline)", len(rustSet))
	}
	if len(goSet) != 46 {
		t.Fatalf("Go ModelInfo JSON field count = %d, want 46 (pinned baseline)", len(goSet))
	}
